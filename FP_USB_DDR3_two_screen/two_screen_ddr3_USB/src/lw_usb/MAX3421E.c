...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	/***INIT_VARS***/
	BYTE cmd_byte = reg + 2;
	BYTE write_buffer[BUFFER_REG_SIZE];
	int test_val;
	int i;
	//...

	/***CONSTRUCT WRITE BUFFER***/
	for ( i = 0; i < BUFFER_REG_SIZE; i++ )
	{
		write_buffer[i] = 0;
	}
	write_buffer[0] = cmd_byte;
	write_buffer[1] = val;
	...
	/***WRITE***/
	test_val = XSpi_Transfer(&SpiInstance, write_buffer, NULL, BUFFER_REG_SIZE);
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	BYTE size = nbytes + 1;
	BYTE cmd_byte = reg + 2;
	BYTE write_buffer[size];
	int test_val;
	int i;
	//...

	/***CONSTRUCT WRITE BUFFER***/
	for ( i = 1; i < size; i++ )		// +1 offset
	{
	...
	/***WRITE***/
	test_val = XSpi_Transfer(&SpiInstance, write_buffer, write_buffer, size);
	if ( test_val != 0 ) xil_printf("ERROR. MAXbytes_wr @ %d", __LINE__);
	//...
...
BYTE MAXreg_rd(BYTE reg) {
	...
	/***INIT_VARS***/
	BYTE cmd_byte = reg;
	BYTE read_buffer[BUFFER_REG_SIZE];
	BYTE write_buffer[BUFFER_REG_SIZE];
	BYTE ret_val;
	int test_val;
	int i;
	...
	/***READ_VAL***/
	test_val = XSpi_Transfer(&SpiInstance, write_buffer, read_buffer, BUFFER_REG_SIZE);
	if ( test_val != 0 )
	{
		xil_printf("ERROR. MAXreg_rd @ %d", __LINE__);
	}
	//...
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	/***READ_VAL***/
	test_val = XSpi_Transfer(&SpiInstance, write_buffer, read_buffer, size);
	if ( test_val != 0 ) xil_printf("ERROR. MAXreg_rd @ %d", __LINE__);
	//...
